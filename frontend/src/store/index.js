...
export default {
  ...
  mutations: {
    // Мутация для изменения состояния ads
    setAds(state, ads) {
      state.ads = ads;
    }
  },
  actions: {
    // Действие для асинхронного получения данных объявлений с сервера и вызова мутации для изменения состояния ads
    async fetchAds({ commit }) {
      try {
        const response = await axios.get("/api/ads"); // Предполагается, что на сервере есть API-маршрут для получения объявлений по адресу /api/ads
